 ...
 * measurements into a single measurement of orientation relative to the Gravity.
 */
...
#define Q ahrs->quaternion.element
    ...
    FusionVector halfFeedback = FUSION_VECTOR_ZERO;
    ...
    if (FusionVectorIsZero(accelerometer) == false) {
        ...
        if ((ahrs->initialising == true) || (FusionVectorMagnitudeSquared(ahrs->halfAccelerometerFeedback) <= ahrs->settings.accelerationRejection)) {
            halfFeedback = ahrs->halfAccelerometerFeedback;
            ahrs->accelerometerIgnored = false;
    ...
    if (FusionVectorIsZero(magnetometer) == false) {
        ...
        halfFeedback = FusionVectorAdd(halfFeedback,ahrs->halfMagnetometerFeedback);
    }
    ...
    const FusionVector adjustedHalfGyroscope = FusionVectorAdd(halfGyroscope, FusionVectorMultiplyScalar(halfFeedback, ahrs->rampedGain));
...
/**
 * @brief Returns the quaternion describing the sensor relative to the Gravity.
 * @param ahrs AHRS algorithm structure.
 * @return Quaternion describing the sensor relative to the Gravity.
 */
 ...
 * measurement with the 1 g of gravity removed. Return vector is in local coordinates.
 * @param ahrs AHRS algorithm structure.
...
/**
 * @brief Returns the acceleration measurement 
      in the Gravity coordinate frame (global coordinates). 
 * @param ahrs AHRS algorithm structure.
 * @return Gravity acceleration measurement in g.
 */
FusionVector FusionAhrsGetGlobalAcceleration(const FusionAhrs *const ahrs) {
#define Q ahrs->quaternion.element
#define A ahrs->accelerometer.axis

    // Calculate accelerometer measurement in the Gravity coordinate frame
...
/**
 * @brief Rotate in the XY plane (around the Z axis) to align our X axis to this target.
 * @param ahrs AHRS algorithm structure.
 * @param targetX New X axis vector, in global coordinates.
 * @return Rotation in radians
 */
float FusionAhrsMatchX(FusionAhrs *const ahrs, FusionVector targetX, float filter) {
    FusionVector curX = FusionQuaternionXaxis(ahrs->quaternion);
    
    // Calculate the error angle (ignore Z axis error, we can't and shouldn't fix that)
    //  This is equal to cross(vec2(curX),vec2(targetX)).z
    float angleRad = curX.axis.x * targetX.axis.y - curX.axis.y * targetX.axis.x;

    // Make a quaternion that fixes this error angle (scaled down by the filter)
    const FusionQuaternion rotation = FusionQuaternionZradians(filter * angleRad);
 ...
 * @param convention Gravity axes convention.
